        ...
        if self.impostor == False:
            ...
            while curr_place.exit != None:
                if isinstance(curr_place.ant, ContainerAnt) and curr_place.ant.contained_ant:
                    self.double_armor(curr_place.ant.contained_ant)
                if curr_place.ant != None:
                    self.double_armor(curr_place.ant)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    previous = bee.action
    current = effect(bee.action, bee)
    def move(gamestate):
        nonlocal duration
        if duration <= 0:
            bee.direction = True
            return previous(gamestate)
        else:
            duration -= 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and not target.scaredbee:
            apply_effect(make_scare, target, 2)
            target.scaredbee = True
        # END Problem EC
